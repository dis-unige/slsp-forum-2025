# Python and Alma's API for printing: Printing slips at circulation desks

Project developed as part of the SLSP Forum 2025 held in Zurich on 27 November 2025

## Participants

* Pablo Iriarte
* Roger Zollinger
* Andreas Ly
* Nicole Meier
* Rouven Schabinger
* Martin Brossard
* Moritz Steinmaier

## Flowchart
![](almaprinter.png)

## Printing projects @ UNIGE
* **Almaprinter**: https://github.com/dis-unige/almaprinter
* **Book Labels Generator with Latex**: https://github.com/dis-unige/book-labels-generator/

## Other alternatives
* Email Client with automatic printing rules
* HoldSlipPrinter - https://developers.exlibrisgroup.com/blog/print-daemon/ 
* Ex Libris Alma Print Daemon - https://developers.exlibrisgroup.com/blog/how-to-set-up-and-use-the-alma-print-daemon/ 
* alma-print-nogui - https://github.com/natliblux/alma-print-nogui 
* Alma Printing With Powershell - https://github.com/university-of-york/fmsys-alma-printing-api
